    ...
    def parse_html_table(self, table):
        ...
        for row in table.find_all('tr'):
            ...
            if len(th_tags) > 0 and len(column_names) == 0:
                for th in th_tags:
                    column_names.append(th.get_text().rstrip().lstrip())
        ...
        for row in table.find_all('tr'):
            ...
            for column in columns:
                df.iat[row_marker, column_marker] = column.get_text().rstrip().lstrip()
                column_marker += 1
            if len(columns) > 0:
                row_marker += 1
...
# print(table.head())
vnrtable = table[table['Visa requirement'].str.contains(vnrtext)]
evisatable = table[table['Visa requirement'].str.contains(evisatext)]
onarrtable = table[table['Visa requirement'].str.contains(onarrtext)]
