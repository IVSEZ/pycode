...
header2 = ["kod1", "firm1", "danno1", "kod2", "firm2", "danno2", "score_tser"]
header3 = ["kod1", "firm1", "danno1", "kod2", "firm2", "danno2", "score_r", "score_pr", "score_tsor", "score_tser", "score_ptsor",
...
with open(file3, 'w', newline='', encoding="utf-8") as outfile3:
    ...
    for line in f:
        ...
        if nin1 != '986-1145-1-0-94' and nin1 != '005-0521-7-1-23' and nin1 != '986-0567-1-1-10':
            # score_r = fuzz.ratio(nin1, nin2)
            # score_pr = fuzz.partial_ratio(nin1, nin2)
            # score_tsor = fuzz.token_sort_ratio(nin1, nin2)
            score_tser = fuzz.token_set_ratio(nin1, nin2)
            # score_ptsor = fuzz.partial_token_sort_ratio(nin1, nin2)
            # score_ptser = fuzz.partial_token_set_ratio(nin1, nin2)
            # score_qr = fuzz.QRatio(nin1, nin2)
            # score_uqr = fuzz.UQRatio(nin1, nin2)
            # score_wr = fuzz.WRatio(nin1, nin2)
            # score_uwr = fuzz.UWRatio(nin1, nin2)
            ...
            if score_tser > 90:
                # temp3 = (
                #     kod1, firm1, nin1, kod2, firm2, nin2, score_r, score_pr, score_tsor, score_tser, score_ptsor,
                #     score_ptser, score_qr,
                #     score_uqr, score_wr, score_uwr)

                temp3 = (kod1, firm1, nin1, kod2, firm2, nin2, score_tser)
